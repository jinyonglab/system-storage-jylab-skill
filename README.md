...
面向 macOS / Windows 的专业只读存储治理技能包。它帮助Codex、claude code等在用户磁盘空间不足时完成只读扫描、目录语义分析、风险分级、证据化处置建议生成，并输出可交互 HTML 工作台。

我按“可直接发布到个人 GitHub 仓库”的标准整理这个仓库：默认不包含运行产物、缓存文件、机器本地路径快照或样例隐私数据。
...
- 将可处理项目依据风险等级分为三类：🟢 可自动清理、🟡 需人工判断、🔴 谨慎清理。
...
- 我知道 `scan.py` 生成的扫描结果会包含当前机器的环境信息，例如用户名、主目录路径、磁盘信息和目录结构摘要。
- 我也知道 `analysis.json` 与最终 HTML 报告会继承上述环境信息，并进一步写入本机目录证据、清理建议和风险判断。
- 因此，我只把这些运行产物用于本地，不会把它们直接当作公开仓库样例提交。
- 本仓库的 `.gitignore` 已默认忽略常见扫描结果、分析结果、工作台报告和缓存文件；在发布前，我仍会人工确认没有把临时产物复制进仓库。
- 如果我确实需要公开展示报告样例，我会先人工脱敏，去除用户名、主目录、真实磁盘卷标、业务项目路径和聊天/同步软件痕迹。
